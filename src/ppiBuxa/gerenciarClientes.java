package ppiBuxa;

import java.util.*;
import java.text.DecimalFormat;

class gerenciarClientes {
    public static void main(String[] args) throws InterruptedException {
        boolean verificacao = true;
        Scanner ler = new Scanner(System.in);

        while (verificacao) {
            System.out.println("====================================");
            System.out.println("GERENCIAR CLIENTES");
            System.out.println("1- Adicionar Cliente");
            System.out.println("2- Editar cliente");
            System.out.println("3- Excluir cliente");
            System.out.println("4- Ver clientes");
            System.out.println("5- Buscar cliente");
            System.out.println("0- Sair");
            System.out.println("====================================");
            System.out.println("Escolha a opção desejada:");

            int opcao = ler.nextInt();

            if (opcao == 1) {
                System.out.println("Digite o nome do cliente:");
                String nome = ler.next();
                System.out.println("Digite o CPF do cliente:");
                String cpf = ler.next();
                System.out.println("Cliente adicionado: " + nome + " CPF: " + cpf);
            }

            if (opcao == 0) {
                verificacao = false;
            }
        }
    }
}

public class JOALHERIAMODELO {
    // Login
    public boolean isUserValid(String usuario) {
        if (usuario == null || usuario.trim().isEmpty()) {
            System.out.println("Usuário não fornecido!");
            return false;
        }
        if (!"admin".equalsIgnoreCase(usuario.trim())) {
            System.out.println("Usuário inválido!");
            return false;
        }
        return true;
    }

    public boolean isPasswordValid(String senha) {
        if (senha == null || senha.trim().isEmpty()) {
            System.out.println("Senha não fornecida!");
            return false;
        }
        if (!"1234".equals(senha.trim())) {
            System.out.println("Senha incorreta!");
            return false;
        }
        return true;
    }

    // -----------------------
    // Dados de estoque e vendas (uso compartilhado)
    // -----------------------
    String[] nome_produto = new String[10000];
    int[] quantidade = new int[10000];
    String[] nome_fornecedor = new String[10000];
    int[] cod_produto = new int[10000];
    int[] peso = new int[10000];
    double[] valor_bruto = new double[10000];
    double[] custo_total = new double[10000];
    double[] valor_final = new double[10000];
    int[] banhoEscolhido = new int[10000];

    String[] codigosBanho = { "P5", "O3", "O7" };
    double[] valorBanho = { 0, 0, 0 };

    int[] codProdutoVenda = new int[10000];
    double[] valorVenda = new double[10000];
    String[] nomeCliente = new String[10000];
    String[] contatoCliente = new String[10000];

    // -----------------------
    // Classe Joia para listagem
    // -----------------------
    static class Joia {
        private int id;
        private String nome;
        private String material;
        private double preco;

        Joia(int id, String nome, String material, double preco) {
            this.id = id;
            this.nome = nome;
            this.material = material;
            this.preco = preco;
        }

        public int getId() { return id; }
        public String getNome() { return nome; }
        public String getMaterial() { return material; }
        public double getPreco() { return preco; }

        @Override
        public String toString() {
            DecimalFormat df = new DecimalFormat("#,##0.00");
            return String.format("%3d | %-20s | %-10s | R$ %8s", id, nome, material, df.format(preco));
        }
    }

    // -----------------------
    // Entrada principal
    // -----------------------
    public static void main(String[] args) {
        JOALHERIAMODELO app = new JOALHERIAMODELO();
        Scanner scanner = new Scanner(System.in);

        // Login
        System.out.print("Digite o usuário: ");
        String usuario = scanner.nextLine().trim();
        if (!app.isUserValid(usuario)) {
            //scanner.close();
            return;
        }
        System.out.print("Digite a senha: ");
        String senha = scanner.nextLine().trim();
        if (!app.isPasswordValid(senha)) {
            //scanner.close();
            return;
        }
        System.out.println("Login realizado com sucesso!");

        // Menu principal unificado
        boolean running = true;
        while (running) {
            System.out.println("\n====================================");
            System.out.println("ACESSO PRINCIPAL");
            System.out.println("1 - Listar / Buscar Joias");
            System.out.println("2 - Gerenciar Estoque e Vendas");
            System.out.println("0 - Sair");
            System.out.println("====================================");
            System.out.print("Escolha a opção desejada: ");
            String op = scanner.nextLine().trim();
            switch (op) {
                case "1":
                    app.listarMenu(scanner); // listar tem opção de ir ao estoque
                    break;
                case "2":
                    try {
                        app.estoqueMenu(scanner);
                    } catch (InterruptedException e) {
                        System.out.println("Interrompido.");
                    }
                    break;
                case "0":
                    running = false;
                    break;
                default:
                    System.out.println("Opção inválida.");
            }
        }

        System.out.println("Encerrando aplicação.");
        scanner.close();
    }

    
    // Menu de listagem (possui opção para ir ao estoque)

    private void listarMenu(Scanner sc) {
        List<Joia> joias = new ArrayList<>();
        joias.add(new Joia(1, "Anel Solitário", "Ouro 18k", 1250.50));
        joias.add(new Joia(2, "Brinco Pérola", "Prata", 299.90));
        joias.add(new Joia(3, "Colar Diamante", "Ouro Branco", 7500.00));
        joias.add(new Joia(4, "Pulseira Coração", "Ouro 14k", 850.00));

        while (true) {
            System.out.println("\n--- Sistema de Listagem de Joias ---");
            System.out.println("1 - Listar todas as joias");
            System.out.println("2 - Filtrar por material");
            System.out.println("3 - Ordenar por preço (asc)");
            System.out.println("9 - Ir para Estoque");
            System.out.println("0 - Voltar");
            System.out.print("Escolha uma opção: ");
            String opt = sc.nextLine().trim();

            if (opt.equals("0")) {
                break;
            } else if (opt.equals("1")) {
                System.out.println("\nID  | Nome                 | Material   | Preço");
                System.out.println("----+----------------------+------------+-----------");
                joias.forEach(j -> System.out.println(j));
            } else if (opt.equals("2")) {
                System.out.print("Digite o material (ex: Ouro 18k, Prata): ");
                String material = sc.nextLine().trim().toLowerCase();
                System.out.println("\nID  | Nome                 | Material   | Preço");
                System.out.println("----+----------------------+------------+-----------");
                joias.stream()
                     .filter(j -> j.getMaterial().toLowerCase().contains(material))
                     .forEach(j -> System.out.println(j));
            } else if (opt.equals("3")) {
                System.out.println("\nOrdenando por preço (ascendente):");
                System.out.println("ID  | Nome                 | Material   | Preço");
                System.out.println("----+----------------------+------------+-----------");
                joias.stream()
                     .sorted(Comparator.comparingDouble(Joia::getPreco))
                     .forEach(j -> System.out.println(j));
            } else if (opt.equals("9")) {
                try {
                    estoqueMenu(sc);
                } catch (InterruptedException e) {
                    System.out.println("Interrompido.");
                }
            } else {
                System.out.println("Opção inválida.");
            }
        }
    }
    // Menu de estoque (adaptado do gerenciarJoias)
    private void estoqueMenu(Scanner estoque) throws InterruptedException {
        boolean verificacao = true;

        while (verificacao) {
            System.out.println("====================================");
            System.out.println("CONTROLE DE ESTOQUE E VENDAS");
            System.out.println("1- Adicionar produto");
            System.out.println("2- Editar produto");
            System.out.println("3- Excluir produto");
            System.out.println("4- Ver estoque");
            System.out.println("5- Buscar produto");
            System.out.println("6- Atualizar BANHO");
            System.out.println("7- Registrar venda");
            System.out.println("0- Voltar");
            System.out.println("====================================");
            System.out.print("Escolha a opção desejada: ");
            String s = estoque.nextLine().trim();
            int opcao;
            try {
                opcao = Integer.parseInt(s);
            } catch (NumberFormatException e) {
                System.out.println("Entrada inválida.");
                continue;
            }

            if (opcao == 1) {
                System.out.println("Digite o nome do fornecedor:");
                String fornecedor = estoque.nextLine();

                System.out.println("Digite o código do produto:");
                int codigo = Integer.parseInt(estoque.nextLine());

                System.out.println("Digite o nome do produto:");
                String nome = estoque.nextLine();

                System.out.println("Digite o peso da joia:");
                int pesoJoia = Integer.parseInt(estoque.nextLine());

                System.out.println("Digite a quantidade:");
                int quant = Integer.parseInt(estoque.nextLine());

                System.out.println("Digite o valor bruto:");
                double bruto = Double.parseDouble(estoque.nextLine());

                System.out.println("Digite a porcentagem de lucro desejada (ex: 20):");
                double lucro = Double.parseDouble(estoque.nextLine());

                System.out.println("Escolha o banho para este produto:");
                for (int i = 0; i < codigosBanho.length; i++) {
                    System.out.println((i + 1) + " - " + codigosBanho[i] + " (R$ " + valorBanho[i] + ")");
                }
                System.out.print("Opção: ");
                int banho = Integer.parseInt(estoque.nextLine()) - 1;

                for (int i = 0; i <= nome_produto.length - 1; i++) {
                    if (nome_produto[i] == null || quantidade[i] == -1) {
                        nome_fornecedor[i] = fornecedor;
                        cod_produto[i] = codigo;
                        nome_produto[i] = nome;
                        peso[i] = pesoJoia;
                        quantidade[i] = quant;
                        valor_bruto[i] = bruto;
                        banhoEscolhido[i] = banho;

                        custo_total[i] = (valorBanho[Math.max(0, Math.min(banho, valorBanho.length-1))] * pesoJoia) + bruto;
                        valor_final[i] = custo_total[i] + (custo_total[i] * (lucro / 100));

                        System.out.println("Produto adicionado na posição " + i);
                        break;
                    }
                }
            }

            if (opcao == 2) {
                System.out.println("Digite o código do produto que deseja editar:");
                int buscar = Integer.parseInt(estoque.nextLine());

                for (int i = 0; i <= cod_produto.length - 1; i++) {
                    if (cod_produto[i] == buscar && nome_produto[i] != null) {
                        System.out.println("Novo fornecedor:");
                        nome_fornecedor[i] = estoque.nextLine();

                        System.out.println("Novo nome:");
                        nome_produto[i] = estoque.nextLine();

                        System.out.println("Novo peso:");
                        peso[i] = Integer.parseInt(estoque.nextLine());

                        System.out.println("Nova quantidade:");
                        quantidade[i] = Integer.parseInt(estoque.nextLine());

                        System.out.println("Novo valor bruto:");
                        valor_bruto[i] = Double.parseDouble(estoque.nextLine());

                        System.out.println("Porcentagem de lucro:");
                        double lucro = Double.parseDouble(estoque.nextLine());

                        int banho = banhoEscolhido[i];
                        custo_total[i] = (valorBanho[banho] * peso[i]) + valor_bruto[i];
                        valor_final[i] = custo_total[i] + (custo_total[i] * (lucro / 100));

                        System.out.println("Produto atualizado!");
                        break;
                    }
                }
            }

            if (opcao == 3) {
                System.out.println("Digite o código do produto que deseja excluir:");
                int buscar = Integer.parseInt(estoque.nextLine());

                System.out.println("Certeza que deseja excluir? (s/n)");
                String resp = estoque.nextLine();

                if (resp.equalsIgnoreCase("s")) {
                    for (int i = 0; i <= cod_produto.length - 1; i++) {
                        if (cod_produto[i] == buscar) {
                            cod_produto[i] = 0;
                            nome_produto[i] = null;
                            nome_fornecedor[i] = null;
                            peso[i] = 0;
                            valor_bruto[i] = 0;
                            custo_total[i] = 0;
                            valor_final[i] = 0;
                            banhoEscolhido[i] = 0;
                            quantidade[i] = -1;
                            System.out.println("Produto excluído!");
                            break;
                        }
                    }
                }
            }

            if (opcao == 4) {
                System.out.println(
                        "------------------------------------------------------------------------------------------------------------------");
                System.out.printf("| %-15s | %-12s | %-6s | %-20s | %-5s | %-10s | %-12s | %-12s |\n",
                        "FORNECEDOR", "COD PRODUTO", "BANHO", "PRODUTO", "PESO", "BRUTO", "CUSTO TOTAL", "VALOR FINAL");
                System.out.println(
                        "------------------------------------------------------------------------------------------------------------------");

                for (int i = 0; i < nome_produto.length; i++) {
                    if (nome_produto[i] != null && quantidade[i] != -1) {
                        System.out.printf("| %-15s | %-12d | %-6s | %-20s | %-5d | %-10.2f | %-12.2f | %-12.2f |\n",
                                nome_fornecedor[i], cod_produto[i], codigosBanho[Math.max(0, Math.min(banhoEscolhido[i], codigosBanho.length-1))], nome_produto[i],
                                peso[i], valor_bruto[i], custo_total[i], valor_final[i]);
                    }
                }
                System.out.println(
                        "------------------------------------------------------------------------------------------------------------------");
                Thread.sleep(500);
            }

            if (opcao == 5) {
                System.out.println("Digite o código do produto para buscar:");
                int buscar = Integer.parseInt(estoque.nextLine());

                for (int i = 0; i <= cod_produto.length - 1; i++) {
                    if (cod_produto[i] == buscar && nome_produto[i] != null) {
                        System.out.println("Produto encontrado:");
                        System.out.printf(
                                "FORNECEDOR: %s | COD PRODUTO: %d | BANHO: %s | PRODUTO: %s | PESO: %d | BRUTO: %.2f | CUSTO TOTAL: %.2f | VALOR FINAL: %.2f\n",
                                nome_fornecedor[i], cod_produto[i], codigosBanho[banhoEscolhido[i]], nome_produto[i],
                                peso[i], valor_bruto[i], custo_total[i], valor_final[i]);
                        break;
                    }
                }
            }

            if (opcao == 6) {
                System.out.println("BANHOS DISPONÍVEIS:");
                for (int i = 0; i < codigosBanho.length; i++) {
                    System.out.println((i + 1) + " - " + codigosBanho[i] + " (Valor atual: R$ " + valorBanho[i] + ")");
                }

                System.out.println("Digite o código do banho que deseja atualizar (P5/O3/O7):");
                String cod = estoque.nextLine();

                for (int i = 0; i < codigosBanho.length; i++) {
                    if (codigosBanho[i].equalsIgnoreCase(cod)) {
                        System.out.println("Digite o novo valor do banho " + codigosBanho[i] + ":");
                        valorBanho[i] = Double.parseDouble(estoque.nextLine());
                        System.out.println("Banho atualizado com sucesso!");
                        break;
                    }
                }
            }

            if (opcao == 7) {
                System.out.println("Digite o código do produto que deseja vender:");
                int codigoVenda = Integer.parseInt(estoque.nextLine());

                int indiceProduto = -1;
                for (int i = 0; i < cod_produto.length; i++) {
                    if (cod_produto[i] == codigoVenda && nome_produto[i] != null) {
                        indiceProduto = i;
                        break;
                    }
                }

                if (indiceProduto == -1) {
                    System.out.println("Produto não encontrado!");
                    continue;
                }

                System.out.println("Escolha o banho para esta venda (P5/O3/O7):");
                String banhoVenda = estoque.nextLine();
                double valorBanhoVenda = 0;
                for (int i = 0; i < codigosBanho.length; i++) {
                    if (codigosBanho[i].equalsIgnoreCase(banhoVenda)) {
                        valorBanhoVenda = valorBanho[i];
                        break;
                    }
                }

                System.out.println("Nome do cliente:");
                String cliente = estoque.nextLine();
                System.out.println("Contato do cliente:");
                String contato = estoque.nextLine();
                System.out.println("Porcentagem de lucro para esta venda:");
                double Venda = Double.parseDouble(estoque.nextLine());

                double custoVenda = valor_bruto[indiceProduto] + (valorBanhoVenda * peso[indiceProduto]);
                double valorVendaFinal = custoVenda + (custoVenda * (Venda / 100));

                for (int i = 0; i < codProdutoVenda.length; i++) {
                    if (codProdutoVenda[i] == 0) {
                        codProdutoVenda[i] = codigoVenda;
                        valorVenda[i] = valorVendaFinal;
                        nomeCliente[i] = cliente;
                        contatoCliente[i] = contato;
                        System.out.println("Venda registrada! Valor final: R$" + valorVendaFinal);
                        break;
                    }
                }
            }

            if (opcao == 0) {
                System.out.println("Retornando ao menu anterior.");
                verificacao = false;
            }
        }
    }
}
